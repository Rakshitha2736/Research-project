"""
Train baseline LSTM (seed 42) on existing sequence arrays (7 features, v1).
...
def main() -> None:
    ...
    scaler_y = joblib.load(MODELS / "minmax_scaler_y.joblib")

    train_loader = make_loader(X_train, y_train, batch_size=BATCH_SIZE, shuffle=True)
    val_loader = make_loader(X_val, y_val, batch_size=BATCH_SIZE, shuffle=False)
    test_loader = make_loader(X_test, y_test, batch_size=BATCH_SIZE, shuffle=False)

    model = LSTMBaseline(input_size=X_train.shape[-1], hidden_size=64, num_layers=2).to(device)
    ...
    ckpt_path = MODELS / "lstm_baseline_seed42.pt"
    torch.save(
        {
    ...
    ax.set_title("LSTM Baseline Training Curve (seed 42, CUDA)")
    ax.legend()
    plt.tight_layout()
    plt.savefig(FIGURES / "lstm_baseline_training_curve.png", dpi=150)
    plt.close()
    ...
    assert len(test_meta) == len(y_test_mm), (
        f"meta/test length mismatch: {len(test_meta)} vs {len(y_test_mm)}"
    )
    meta_df = pd.DataFrame(test_meta)
    meta_df["y_true"] = y_test_mm
    meta_df["y_pred"] = y_pred_mm
    counts = meta_df["station_id"].value_counts()
    sample_station = counts.index[0]
    ...
    ax.set_title(f"Predicted vs Actual — {sample_station} (test)")
    ax.legend()
    plt.tight_layout()
    plt.savefig(FIGURES / "lstm_baseline_pred_vs_actual_sample.png", dpi=150)
    plt.close()

    with open(MODELS / "lstm_baseline_seed42_metrics.json", "w", encoding="utf-8") as f:
        json.dump(
            {
                "stopped_epoch": stopped_epoch,
                "best_epoch": best_epoch,
    ...
    print(f"stopped_epoch: {stopped_epoch} (best_epoch={best_epoch})")
